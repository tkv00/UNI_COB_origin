...
class PhsicalActivity :AppCompatActivity() , NavigationView.OnNavigationItemSelectedListener{
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        textViewTitle.text = intent.getStringExtra(ARG_TITLE) ?: "예술•체육 기타"
            ...
            override fun onResponse(call: Call<LectureDTO>, response: Response<LectureDTO>) {
                if (response.isSuccessful) {
                    ...
                    val designLectures = response.body()?.data?.filter {
                        it.classification == "예술ㆍ체육기타"
                    } ?: listOf()
                    // 어댑터 데이터 업데이트
                    adapter.updateData(designLectures)
                } else {
                    Log.e("DesignActivity", "Response not successful: ${response.errorBody()?.string()}")
                }
            }

            override fun onFailure(call: Call<LectureDTO>, t: Throwable) {
                Log.e("DesignActivity", "Network request failed", t)
            }
        })
    }
